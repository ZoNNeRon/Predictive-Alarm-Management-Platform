...
Промпт расположен в config/prompts/diagnostic_agent.md.
...
from src.xai.xai_module import SymptomVector, XAIExplainer
...
class DiagnosticAgent:
    ...
    def __init__(self, model_name: Optional[str] = None,
        ...
        # Нативный клиент Ollama позволяет отключать chain-of-thought через
        # think=False (см. generate_prescription). Это надёжнее текстовой
        # директивы /no_think, которая ведёт себя по-разному в сборках Qwen 3.5:
        # иначе модель «думает вслух», ломая формат и тратя минуты на генерацию.
        self.client = ollama.Client(host=ollama_base_url)
        # num_predict ограничивает длину вывода (предписание должно быть компактным);
        # num_ctx - размер окна контекста под 4-канальный промпт.
        self.options = {
    ...
    def _event_header(self, symptom_vector) -> str:
        """
        Детерминированная шапка с тегом агрегата и временем события.
    ...
    def _build_full_prompt(self, symptom_vector, rag_results, schedule_results,
        ...
        if stage == 'warning':
            ...
            verdict = (f"Вероятность аварии составляет {pct:.0f}% - ТРЕБУЕТСЯ "
    ...
    def generate_prescription(self, symptom_vector: SymptomVector,
        ...
        # Промпт (все четыре канала RAG + стадийный вердикт) собирается единой
        # точкой; она же возвращает список источников и признак использования
        # контекста для AgentResponse.
        ...
        return AgentResponse(
            ...
            used_context=used_ctx, sources=sources,
...
if __name__ == "__main__":
    ...
    chroma_dir = os.path.join(project_root, 'artifacts', 'chroma_db')
    ...
    # 3. Agent: генерирует предписание для каждой модели
