...
Детерминированный демо-сценарий для валидации платформы (раздел 3.9).
...
def extract_demo_scenario(
    ...
    for pid in pumps:
        grp = df[df["pump_id"] == pid].reset_index(drop=True)
        st = _state_series(grp).values  # type: ignore
        ft = grp["fault_type"].values
