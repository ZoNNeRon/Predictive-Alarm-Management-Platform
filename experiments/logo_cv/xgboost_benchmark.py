...
# Данная функция дублирует evaluate_model() из src/ml_pipeline.py
# Сделано для исключения необходимости выноса функции в отдельный модуль
...
def comparison_plot(cv_metrics: list, save_dir: str = None):  # type: ignore
    """
    ...
    Args:
        ...
        save_dir:   директория сохранения; по умолчанию data/graphs.
    ...
    means = [np.mean(f1_all), np.mean(pr_auc_all), np.mean(recall_all)]
    stds = [np.std(f1_all),  np.std(pr_auc_all),  np.std(recall_all)]
    ...
    fig.suptitle('Оценка робастности XGBoost на новом оборудовании (Leave-One-Pump-Out CV)',
    ...
    ax_metrics.set_xticklabels(pumps, color=TEXT_CLR, fontsize=10)
    ax_metrics.set_ylim(0.6, 1.05)
    ax_metrics.set_ylabel('Значение метрики', color=TEXT_CLR, fontsize=11)
    ...
    for fold_idx, m in enumerate(cv_plot):
        ...
        ax.set_title(f'{pumps[fold_idx]}', color=TEXT_CLR, fontsize=11,
                     fontweight='bold', pad=6)
        ax.set_xticks(range(3)); ax.set_xticklabels(CLASS_LABELS, fontsize=7, rotation=30)
        ax.set_yticks(range(3))
        # Подписи оси Y только у крайней левой матрицы — экономия места
        if fold_idx == 0:
            ax.set_yticklabels(CLASS_LABELS, fontsize=7)
            ax.set_ylabel('Истинно', color=TEXT_CLR, fontsize=9)
        else:
            ax.set_yticklabels([])
        ax.set_xlabel('Предсказано', color=TEXT_CLR, fontsize=8)
    ...
    fig.text(0.5, 0.46, 'Матрицы ошибок по фолдам (нормировка по строкам, в ячейке — доля и значение)',
             ha='center', color=TEXT_CLR, fontsize=12)
