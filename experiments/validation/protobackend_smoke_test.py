...
==============================================
experiments/validation/protobackend_smoke_test.py
...
from src.runtime.platform_backend import ProtoBackend, TickResult
...
def _proto() -> ProtoBackend:
    """Общий на модуль экземпляр ProtoBackend (кешируется, чтобы не пересоздавать)."""
...
def _row(pid: str, i: int, state: int, fault) -> pd.Series:
    """Синтетическая строка телеметрии для тика: i задаёт время и лёгкую вариацию
    сенсоров (чтобы std != 0 на полном окне); state/fault - метки FSM."""
...
def _drive_until_ready(backend, pid, state, fault, n=WARMUP_ROWS + 5) -> TickResult:
    """Прогнать n строк через process_tick; вернуть последний (готовый) tick.

    assert гарантирует непустой tick (n >= 1) - и снимает у вызывающих
    reportOptionalMemberAccess при доступе к tick.ready/tick.severity."""

    tick: Optional[TickResult] = None
    for i in range(n):
        tick = backend.process_tick(pid, _row(pid, i, state, fault))
    assert tick is not None, "n должно быть >= 1, иначе tick не вычислен"
    return tick
