"""
Блок визуализации результатов обучения ML-моделей
=================================================
src/visualisation/ml_visualisation.py

Светлая (академическая) тема. Три публичные функции - обе модели иерархии:

plot_all(metrics_list, y_test, output_dir) - модель ТЯЖЕСТИ (severity):
    - ML_plot1_confusion_matrices.png - матрицы ошибок 3 моделей (LR/RF/XGBoost);
    - ML_plot2_metrics_comparison.png - сравнение по F1/Recall/PR-AUC (grouped bar);
    - ML_plot3_pr_curves_critical.png - PR-кривые класса «Авария».

recall_plot(results, signatures, save_dir) - валидация различения сигнатур:
    - ML_plot4_fault_recall_analysis.png - слева Recall по типам отказа
      (Critical и Warning), справа тепловая карта сигнатур датчиков (Critical).

plot_fault_classifier(metrics_list, output_dir, best_label=None) -
классификатор ТИПА отказа (вторая модель):
    - ML_fault_plot1_confusion_matrix.png - матрицы ошибок по моделям;
    - ML_fault_plot2_model_comparison.png - macro-F1 / balanced accuracy;
    - ML_fault_plot3_stage_split.png - Recall раздельно Warning vs Critical.
"""
...
def _apply_light_style(ax, title: str = ''):
    ...
    if title:
        ax.set_title(title, color=TEXT_CLR, fontsize=18, fontweight='bold', pad=10)


def plot_all(metrics_list: list, y_test: pd.Series, output_dir: str):
    """Все графики раздела в одной функции"""
    ...
    fig.suptitle('Матрицы ошибок (тестовая выборка: насос MNHV_005)',
                 color=TEXT_CLR, fontsize=22, fontweight='bold', y=1.05)
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(metric_labels, color=TEXT_CLR, fontsize=14)
    ax.set_ylim(0.35, 1.05) # Расширили верхний лимит для подписей
    ax.set_ylabel('Значение метрики', color=TEXT_CLR, fontsize=14)
    ...
    ax.set_title('Сравнение качества моделей по ключевым метрикам\n'
                 ...
                 color=TEXT_CLR, fontsize=18, fontweight='bold')
    ...
    for m, color in zip(metrics_list, colors):
        prec, rec, _ = precision_recall_curve(y_test_bin[:, 2], m['y_proba'][:, 2])
    ...
    baseline = y_test_bin[:, 2].mean()
    ax.axhline(baseline, color='#888888', lw=1.2, ls='--',
               label=f'Случайный классификатор (P = {baseline:.3f})', zorder=1)

    ax.set_xlabel('Recall (Полнота обнаружения аварий)', color=TEXT_CLR, fontsize=14)
    ax.set_ylabel('Precision (Точность предупреждений)', color=TEXT_CLR, fontsize=14)
    ax.tick_params(axis='both', labelsize=12)
    ax.set_title('PR-кривые для класса «Авария» (класс 2)\n'
...
def recall_plot(results: list, signatures, save_dir: str):
    ...
    ax.legend(fontsize=10, framealpha=0.9, facecolor='#FFFFFF', edgecolor='#CCCCCC')
    ...
    if signatures is not None and not signatures.isna().all().all():
        ...
        ax2.set_xticklabels(col_names, fontsize=14)
        ax2.set_yticks(range(len(FAULT_TYPES)))
        ax2.set_yticklabels([FAULT_LABELS[ft] for ft in FAULT_TYPES], fontsize=12)
        ...
        for i in range(len(FAULT_TYPES)):
            for j in range(sig_vals.shape[1]):
                ...
                ax2.text(j, i, f'{val:.1f}',
                         ha='center', va='center',
                         fontsize=14, fontweight='bold', color=txt_clr)
...
def plot_fault_classifier(metrics_list: list, output_dir: str,
    ...
    fig.suptitle('Матрицы ошибок классификатора типа отказа (тест: MNHV_005)',
                 color=TEXT_CLR, fontsize=24, fontweight='bold', y=1.05)
    for ax, m in zip(axes, metrics_list):
        ...
        _apply_light_style(ax, m['label'])
        ax.set_xlabel('Предсказано', color=TEXT_CLR, fontsize=16)
        ax.set_ylabel('Истинно', color=TEXT_CLR, fontsize=16)
    ...
    for i, m in enumerate(metrics_list):
        ...
        for b, v in zip(bars, vals):
            ax.text(b.get_x() + b.get_width() / 2, v + 0.005, f'{v:.3f}',
                    ha='center', va='bottom', fontsize=14, fontweight='bold', color=TEXT_CLR)
    ax.set_xticks(x)
    ax.set_xticklabels(metric_labels, color=TEXT_CLR, fontsize=14)
    ...
    ax.legend(framealpha=1.0, labelcolor=TEXT_CLR, fontsize=14,
              facecolor=LIGHT_BG, edgecolor='#CCCCCC')
    for sp in ax.spines.values():
        sp.set_edgecolor('#CCCCCC')
    plt.tight_layout()
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([FAULT_LABELS[ft] for ft in FAULT_TYPES], 
                       fontsize=14, color=TEXT_CLR)
    ax.tick_params(axis='y', labelsize=12)
    ax.set_ylim(0.8, 1.18)
    ax.set_ylabel('Recall', color=TEXT_CLR, fontsize=14)
