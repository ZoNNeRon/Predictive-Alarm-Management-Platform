"""
Runtime-слой управления тревогами (Alarm Runtime)
=================================================
src/runtime/alarm_runtime.py

Слой управления тревогами поверх предсказаний ML:
...
def _sev_label(state: Optional[int]) -> str:
    """Метка тяжести по коду состояния; '?' для None или неизвестного кода."""
    
    if state is None:
        return "?"
    return SEVERITY_LABELS.get(state, "?")


@dataclass
class JournalEvent:
    """Одна запись журнала тревог: что, когда и с каким агрегатом произошло.

    Атомарный факт для архива и аудита (переход состояния, подавленный сигнал,
    квитирование, сброс). Тип события задаётся полем kind; human-readable
    описание собирает свойство label.
    """
    ...
    def label(self) -> str:
        if self.kind == "transition":
            return f"{_sev_label(self.from_state)} -> {_sev_label(self.to_state)}"
...
class AlarmJournal:
    """Хронологический архив всех событий тревог парка (append-only).

    Хранит и подтверждённые переходы, и подавленные state-based фильтрацией
    сигналы (требование ФЗ № 116-ФЗ / ГОСТ Р 22.1.12-2005: скрытое не теряется).
    Кольцевое ограничение maxlen защищает от роста памяти на долгом прогоне;
    методы last()/count() отдают срез для UI и статистику по типам событий.
    """
